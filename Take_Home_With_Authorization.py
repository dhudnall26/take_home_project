...
#oath2 security
oauth_login = OAuth2PasswordBearer(tokenUrl = "token")

# declares POST method to obtain token needed for meal information requests
@app.post("/token")
async def token_generate(authorize_data: OAuth2PasswordRequestForm = Depends()):
    if ((authorize_data.username == user_name) and (authorize_data.password == password)):
        return {"access_token": "food", "token_type": "bearer"}
    else:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail=f'Sorry, you have the wrong username or password.')

# declares GET method to obtain available meals on given dates
@app.get("/menu/{date_id}/{type_id}", status_code=status.HTTP_200_OK)
async def read_item(date_id, type_id, token: str = Depends(oauth_login)):
# verifies token input from user is correct
    if token != "food":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f'Sorry, you have the wrong token.')
